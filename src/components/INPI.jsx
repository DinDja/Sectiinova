            ...
            <div className="flex items-center justify-between gap-4 mb-3">
                ...
                <button
                    onClick={() => {
                        const json = localStorage.getItem('inpi_agent_autodoc');
                        if (!json) {
                            alert('Nenhum documento gerado encontrado. Execute o Agente IA primeiro.');
                            return;
                        }
                        try {
                            const data = JSON.parse(json);
                            setFormData((prev) => ({ ...prev, ...data }));
                            alert('Conteúdo carregado no Gerador de Documentos.');
                        } catch (err) {
                            console.error(err);
                            alert('Falha ao interpretar dados de documento.');
                        }
                    }}
