...
const MentorBadge = ({ person, getLattesLink }) => {
    ...
    return (
        <article className="group relative min-w-[280px] flex-1 rounded-3xl border border-slate-100 bg-white p-6 shadow-sm hover:shadow-xl hover:shadow-cyan-500/5 hover:border-[#10B981]/20 transition-all duration-500 overflow-hidden"
      > 
            
            <div className="flex items-start justify-between gap-4 relative z-10">
                <div className="flex items-center gap-3.5">
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-slate-50 text-sm font-black text-slate-600 border-2 border-white shadow-sm group-hover:bg-[#10B981] group-hover:text-white transition-colors duration-300">
                        {getInitials(person.nome)}
                    </div>
                    <div>
                        <h5 className="text-sm font-bold text-slate-900 leading-tight group-hover:text-[#10B981] transition-colors">{person.nome}</h5>
                        <p className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 mt-0.5">{person.perfil || 'Mentoria'}</p>
                    </div>
                </div>
                {lattesLink ? (
                    <a href={lattesLink} target="_blank" rel="noreferrer" className="inline-flex shrink-0 items-center justify-center w-10 h-10 rounded-xl border border-[#10B981]/20 bg-[#ECFDF5] text-[#10B981] transition-all hover:bg-[#10B981] hover:text-white shadow-sm" title="Abrir Lattes">
                ...
                <div className="mt-5 space-y-4 relative z-10 border-t border-slate-100 pt-5">
                    {summary && (
                        <div className="rounded-2xl bg-slate-50 border border-slate-100 px-4 py-3 text-xs leading-relaxed text-slate-600 group-hover:bg-white transition-colors">
                            <div className="mb-1.5 flex items-center gap-1.5 font-bold uppercase tracking-widest text-[#047857] text-[10px]">
                    ...
                    {areas.length > 0 && (
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {areas.map((area) => (
                                    <span key={area} className="rounded-lg border border-[#10B981]/10 bg-[#ECFDF5] px-2.5 py-1 text-[10px] font-semibold text-[#047857]">
                                        {area}
                                    </span>
                                ))}
                            </div>
                    ...
                    {education.length > 0 && (
                        <div>
                            ...
                            <div className="space-y-1 text-xs font-medium text-slate-700">
                                {education.map((item) => (
                                    <p key={item} className="flex items-start gap-1.5"><span className="text-[#10B981] mt-0.5">•</span> {item}</p>
                                ))}
                            </div>
                        </div>
                    )}
...
const DiaryEntryCard = ({ entry }) => (
    <div className="group relative bg-white border border-slate-100 rounded-[2rem] hover:shadow-2xl hover:shadow-cyan-500/5 hover:border-[#10B981]/30 transition-all duration-500 overflow-hidden w-full">
        <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-gradient-to-b from-[#10B981] to-transparent opacity-50 group-hover:opacity-100 transition-opacity"></div>
        ...
        <div className="bg-slate-50/50 border-b border-slate-100 px-8 py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h4 className="text-xl font-extrabold text-slate-900 tracking-tight mb-2 group-hover:text-[#10B981] transition-colors">{entry.title}</h4>
                <div className="flex flex-wrap items-center text-xs font-semibold text-slate-500 gap-4">
                    <span className="flex items-center gap-1.5 bg-white px-2.5 py-1 rounded-md border border-slate-200 shadow-sm"><Calendar className="w-3.5 h-3.5 text-[#10B981]" /> {entry.date}</span>
            ...
            <div className="inline-flex items-center gap-2 bg-[#ECFDF5] text-[#047857] border border-[#10B981]/20 px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest shadow-inner shrink-0">
    ...
    if (!selectedProject) {
        return (
            <div className="min-h-[50vh] flex items-center justify-center p-10 bg-white rounded-[3rem] border border-slate-100 shadow-inner relative overflow-hidden">
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-[#10B981]/5 rounded-full blur-[100px] pointer-events-none"></div>
        ...
        return (
            <div className="min-h-[50vh] flex items-center justify-center p-10 bg-white rounded-[3rem] border border-slate-100 shadow-inner relative overflow-hidden">
                ...
                <div className="relative z-10 text-center">
                    <EmptyState
                        icon={BookOpen}
                        title="Acesso restrito ao Diário"
    ...
    return (
        <div className="space-y-10 mx-auto font-sans text-slate-800 max-w-7xl pt-5">
            ...
            <section className="relative overflow-hidden bg-white border border-slate-100 rounded-[2.5rem] p-8 md:p-10 shadow-sm">
                <div className="absolute -top-24 -right-24 w-96 h-96 bg-[#10B981]/5 rounded-full blur-[80px] pointer-events-none"></div>
                ...
                <div className="relative z-10">
                    ...
                    <div className="bg-slate-50/80 rounded-3xl p-6 border border-slate-100 mb-8 max-w-4xl shadow-inner">
                        <div className="flex items-center gap-2 mb-3 text-[#10B981] font-black uppercase tracking-widest text-xs">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                        <div className="bg-white border border-slate-200 rounded-2xl p-4 flex flex-col justify-center hover:border-[#10B981]/30 transition-colors shadow-sm">
                        ...
                        <div className="bg-white border border-slate-200 rounded-2xl p-4 flex flex-col justify-center hover:border-[#10B981]/30 transition-colors shadow-sm">
                            ...
                            <span className="text-sm font-bold text-[#10B981] flex items-center gap-2">
                                <span className="w-2 h-2 rounded-full bg-[#10B981] animate-pulse"></span> {selectedProject.status || 'Não informado'}
                            </span>
                        </div>
                        <div className="bg-white border border-slate-200 rounded-2xl p-4 flex flex-col justify-center hover:border-[#10B981]/30 transition-colors shadow-sm md:col-span-2">
                    ...
                    {uniqueMentors.length > 0 && (
                        <div className="pt-6 border-t border-slate-100">
                            <h4 className="text-xs font-black text-slate-800 uppercase tracking-widest mb-4 flex items-center gap-2">
                                <GraduationCap className="w-4 h-4 text-[#10B981]" /> Equipe de Orientação
                            </h4>
            ...
            <section className="bg-white rounded-[3rem] p-6 md:p-10 border border-slate-100 shadow-inner">
                ...
                <div className="space-y-6">
                    {derivedDiaryEntries.length === 0 ? (
                        <div className="bg-white border-2 border-dashed border-slate-200 rounded-[2.5rem] p-12 text-center hover:border-[#10B981]/30 transition-colors">
                            <EmptyState 
                                icon={BookOpen} 
                        ...
                        <div className="relative border-l-2 border-slate-200 ml-4 md:ml-6 space-y-10 pb-6">
                            {derivedDiaryEntries.map((entry) => (
                                <div key={entry.id} className="group relative pl-6 md:pl-10">
                                    ...
                                    <div className="absolute -left-[9px] top-8 w-4 h-4 rounded-full bg-[#10B981] ring-4 ring-slate-50 shadow-sm z-10 transition-transform duration-300 group-hover:scale-125 group-hover:ring-[#10B981]/20"></div>
