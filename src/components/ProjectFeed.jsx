...
export default function ProjectFeed({
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8 pb-12 px-4 sm:px-0">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 pb-4 border-b border-slate-200/80 animate-fade-in">
                <div>
                    <h1 className="text-3xl md:text-4xl font-black text-slate-800 tracking-tight leading-tight">
                        Inovação em <span className="premium-gradient-text">Foco</span>
                    </h1>
                ...
                <div className="flex gap-3">
                    <div className="premium-chip" title="Total de escolas participantes">
                        <School className="w-4 h-4 text-slate-400" />
                        <span className="text-sm font-semibold text-slate-700">{schools.length}</span>
                        <span className="text-xs text-slate-400 hidden sm:inline">escolas</span>
                    </div>
                    <div className="premium-chip" title="Total de projetos cadastrados">
                        <FolderKanban className="w-4 h-4 text-slate-400" />
                        <span className="text-sm font-semibold text-slate-700">{projectsTotalCount}</span>
                        <span className="text-xs text-slate-400 hidden sm:inline">projetos</span>
                    </div>
                </div>
            </div>
            ...
            <div className="pt-2 pb-6 flex flex-col items-center gap-4">
                ...
                {/* Caso haja erro no carregamento (poderíamos adicionar um estado de erro) */}
                {/* Exemplo: se error, mostrar mensagem com botão de retry */}
                {/* Não incluso pois não há prop de erro, mas é uma boa prática */}
            </div>
        </div>
    );
}